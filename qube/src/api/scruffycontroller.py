...
from qube.src.api.swagger_models.scruffy import ScruffyModel # noqa: ignore=I100
from qube.src.api.swagger_models.scruffy import ScruffyModelPost # noqa: ignore=I100
from qube.src.api.swagger_models.scruffy import ScruffyModelPostResponse # noqa: ignore=I100
from qube.src.api.swagger_models.scruffy import ScruffyModelPut # noqa: ignore=I100
...
from qube.src.api.swagger_models.response_messages import (
    ...
    put_response_msgs)
from qube.src.commons.error import ScruffyServiceError
...
class ScruffyItemController(Resource):
    @swagger.doc(
        {
            'tags': ['Scruffy'],
            'description': 'Scruffy get operation',
    ...
    def get(self, authcontext, entity_id):
        ...
        try:
            ...
            data = ScruffyService(authcontext['context'])\
                .find_by_id(entity_id)
            clean_nonserializable_attributes(data)
        except ScruffyServiceError as e:
            LOG.error(e)
        ...
        return ScruffyModel(**data), 200

    @swagger.doc(
        {
            'tags': ['Scruffy'],
            'description': 'Scruffy put operation',
    ...
    def put(self, authcontext, entity_id):
        """
        updates an scruffy item
        """
        try:
            model = ScruffyModelPut(**request.get_json())
            context = authcontext['context']
            ScruffyService(context).update(model, entity_id)
            return EMPTY, 204
        except ScruffyServiceError as e:
            LOG.error(e)
            return ErrorModel(**{'error_code': str(e.errors.value),
    ...
    @swagger.doc(
        {
            'tags': ['Scruffy'],
            'description': 'Scruffy delete operation',
    ...
    def delete(self, authcontext, entity_id):
        ...
        """
        try:
            ScruffyService(authcontext['context']).delete(entity_id)
            return EMPTY, 204
        except ScruffyServiceError as e:
            LOG.error(e)
            return ErrorModel(**{'error_code': str(e.errors.value),
...
class ScruffyController(Resource):
    @swagger.doc(
        {
            'tags': ['Scruffy'],
            'description': 'Scruffy get operation',
    ...
    def get(self, authcontext):
        """
        gets all scruffy items
        """
        LOG.debug("Serving  Get all request")
        list = ScruffyService(authcontext['context']).get_all()
    ...
    @swagger.doc(
        {
            'tags': ['Scruffy'],
            'description': 'Scruffy create operation',
    ...
    def post(self, authcontext):
        ...
        """
        try:
            model = ScruffyModelPost(**request.get_json())
            result = ScruffyService(authcontext['context'])\
                .save(model)

            response = ScruffyModelPostResponse()
