...
class ScruffyService:
    ...
    def find_by_id(self, entity_id):
        ...
        data = Scruffy.query.get(entity_id)
        if data is None:
            raise ScruffyServiceError(
                'scruffy {} not found'.format(entity_id),
    ...
    def get_all(self):
        list = []
        data = Scruffy.query.filter(
            Scruffy.tenantId == self.auth_context.tenant_id)
    ...
    def update(self, model, entity_id):
        ...
        record = Scruffy.query.get(entity_id)  # Scruffy is a mongo class
        if record is None:
            raise ScruffyServiceError(
                'scruffy {} not found'.format(entity_id),
                ErrorCodes.NOT_FOUND)
